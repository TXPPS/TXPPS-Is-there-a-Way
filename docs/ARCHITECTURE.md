...
Everything else in this game is used where it stands. A tool is the first thing
that moves through the building with the player, and P3.2 needs one: the
photometer is worthless in the room it was found in.

`Hands` hangs under the camera and holds at most one thing, by **reparenting**
rather than by a flag. A tool held is a tool whose transform *is* the camera's;
anything else means writing a follow behaviour that will disagree with the
camera on some frame. It also means a tool crosses an act boundary in the
player's hand with no special case at all, because the player is not part of the
act — and a tool put down belongs to the act again, so it is freed with it
rather than outliving it invisibly in the middle of the next one.

Putting it down is the **action button with nothing to act on**. That press
already did nothing, and this is the obvious thing for it to mean once the
player's hands can be full; no new control, no new reserved rect.

The tool's *job* is a child node with a `read()` on it, so a second instrument
is a second child rather than a second class.

**`Photometer` is the game's one moment of instrumented proof**, and it gets
there without knowing the entity exists. A reading is the sum over every lit
practical that can *see* the meter — and the entity's whole existence is
standing between the player and a lamp. When it does, that lamp stops counting
and the reading falls by exactly what that lamp was worth. The drop is not
noise: it is a number, and it is the same number every time. Deliberately the
same idea as the debug overlay — on a device with no console, a number you can
read beats a feeling you cannot.

The calibration constant is a *certificate*, not a physical constant. Engine
light energy is not lux and nothing in Godot says what it is; the number here is
chosen so a sodium bulkhead at two metres reads about 120 lx. Which is why
`D-18` has forty-four quarterly entries in it.
...
error rather than an inconvenience. The pause menu lists what has been read and
opens any of it in the same reader the world uses, so putting a page down does
what putting a page down always does.

The journal saves **ids**, because ids are small and stable and a save should
not carry a copy of the text — which means after a reload it knows twenty-two
ids and cannot show one of them. `DocumentIndex` is where they come back from:
generated by `tools/gen/make_doc_index.py` from what is on disk, and checked by
the suite *against* what is on disk, because a document that exists and is not
indexed is one the player can find once and then lose, and nothing else would
notice.

The list never shows a title the player has not found. What exists is itself
information.
