...
| **Stamp to expect** | `v0.1.0 c8ed261` (or later — every push deploys) |
...
| | Status |
|---|---|
| Build | **green.** Export, budgets (download, texture size, shipped audio), the import-settings check, a **532-check headless suite**, 55-check gameplay smoke, 28-check update-path smoke. |
| Last verified | `v0.1.0 c8ed261`, serving from GitHub Pages with every payload file answering 200 and the wasm as `application/wasm`. |
